...
//     fn draw(&self, max_size: Size, request: RequestTree, offset: Offset, bound: Rect) -> Vec<(CanvasArea, CanvasItem)>;
...
//     fn draw(&self, max_size: Size, request: RequestTree, offset: Offset, bound: Rect) -> Vec<(CanvasArea, CanvasItem)> {
//         Drawable::draw(&**self, max_size, request, offset, bound)
//     }
...
//     fn draw(&self, max_size: Size, request: RequestTree, offset: Offset, bound: Rect) -> Vec<(CanvasArea, CanvasItem)> {
//         self.as_ref().map(|d| d.draw(max_size, request, offset, bound)).unwrap_or_default()
//     }
...
//     fn draw(&self, size: Size, request: RequestTree, offset: Offset, bound: Rect) -> Vec<(CanvasArea, CanvasItem)> {
//         vec![(CanvasArea{offset, bounds: Some(bound)}, CanvasItem::Text(self.clone()))]
//     }
// }
...
//     fn draw(&self, max_size: Size, request: RequestTree, poffset: Offset, bound: Rect) -> Vec<(CanvasArea, CanvasItem)> {
//         let size = request.0.get(max_size);
//         let children = request.1.iter().map(|b| b.0).collect::<Vec<_>>();
//         self.layout().build(size, children).into_iter()
//             .zip(self.children()).zip(request.1)
//             .flat_map(|((Area{offset, size}, child), branch)| {
//                 let poffset = (poffset.0+offset.0, poffset.1+offset.1);
...
//                 if bound.2 != 0.0 && bound.3 != 0.0 {
//                     child.draw(size, branch, poffset, bound)
//                 } else {vec![]}
//             }).collect()
//     }
